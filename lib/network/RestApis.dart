...
Future<void> logout({bool isDelete = false}) async {
  if (!isDelete) {
    try {
      ...
      // Sin red / DNS / servidor: igual cerramos sesión en el dispositivo.
      log('logoutApi: $e');
    }
    await logOutSuccess();
  } else {
...
/// Obtiene la URL para compartir el viaje (solo pasajero).
Future<String> getRideShareUrl({required int rideId}) async {
  final response = await buildHttpResponse('riderequest-share-url', method: HttpMethod.POST, request: {'ride_id': rideId});
  final json = await handleResponse(response);
  final data = json['data'];
  if (data == null || data['share_url'] == null) throw Exception(json['message'] ?? 'Share URL not available');
...
Future<NearByDriverModel> getNearByDriverList({LatLng? latLng, bool useExpandedRadius = false}) async {
  final expanded = useExpandedRadius ? '&use_expanded_radius=1' : '';
  return NearByDriverModel.fromJson(await handleResponse(await buildHttpResponse('near-by-driver?latitude=${latLng!.latitude}&longitude=${latLng.longitude}$expanded', method: HttpMethod.GET)));
}
