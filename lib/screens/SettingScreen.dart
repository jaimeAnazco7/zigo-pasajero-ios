...
class SettingScreenState extends State<SettingScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(language.settings, style: boldTextStyle(color: neonHighlight)),
      ),
  ...
  static const Color _drawerYellow = neonHighlight;
  static const Color _drawerCyan = neonAccent;

  Widget settingItemWidget(IconData icon, String title, Function() onTap, {bool isLast = false, Widget? suffixIcon, Color? color}) {
    final Color itemColor = color ?? _drawerCyan;
    final Color textColor = color ?? _drawerYellow;
        ...
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(border: Border.all(color: itemColor), borderRadius: radius(defaultRadius)),
              child: Icon(icon, size: 20, color: itemColor),
            ),
            SizedBox(width: 12),
            Expanded(child: Text(title, style: primaryTextStyle(color: textColor))),
            suffixIcon != null ? suffixIcon : Icon(Icons.navigate_next, color: itemColor),
          ],
        ),
      ),
    );
